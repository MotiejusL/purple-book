class FriendRequestsController < ApplicationController
  def index
    ...
    friendRequests.each do |request|
      @friendRequestsUsers.push(requestId: request.id, img: request.user.image, img_path: view_context.image_path(request.user.image), name: request.user.firstname + " " + request.user.lastname, userId: request.user.id)
    end
